...
elseif(handles.fileLoaded==1)
    ...
    handles.RGB2(:,:,1) = medfilt2(handles.RGBtmp(:,:,1),[M1 M2]); % Median filtering for Red component
    waitbar(1/3, w);
    handles.RGB2(:,:,2) = medfilt2(handles.RGBtmp(:,:,2),[M1 M2]); % Median filtering for Green component
    waitbar(2/3, w);
    handles.RGB2(:,:,3) = medfilt2(handles.RGBtmp(:,:,3),[M1 M2]); % Median filtering for Blue component
    close(w);
    axes(handles.axes2); imshow(handles.RGB2);
    handles.fileLoaded2 = 1;
    handles = updateHistograms(handles);
...
elseif (handles.fileLoaded==1)
 handles.RGBtmp = handles.RGB;
    ...
    H = fspecial('unsharp');
    handles.RGB2(:,:,1) = imfilter(handles.RGBtmp(:,:,1),H,'replicate');
    handles.RGB2(:,:,2) = imfilter(handles.RGBtmp(:,:,2),H,'replicate');
    handles.RGB2(:,:,3) = imfilter(handles.RGBtmp(:,:,3),H,'replicate');
    axes(handles.axes2); imshow(handles.RGB2);
    handles.fileLoaded2 = 1;
    handles = updateHistograms(handles);
    guidata(hObject, handles);
...
elseif (handles.fileLoaded==1)
 handles.RGBtmp = handles.RGB;
    ...
    handles.RGB2 = changeBrightness(handles.RGBtmp, get(handles.sliderBright, 'Value'), get(handles.sliderContrast, 'Value'));
    axes(handles.axes2); imshow(handles.RGB2);
    handles.fileLoaded2 = 1;
    handles = updateHistograms(handles);
    guidata(hObject, handles);
